updatelatestbodc = function(BODC = list()){
...
library("xml2")
  ...
  write.csv(parameters, 'BODCdata/parameters.csv', row.names = FALSE, na = "" )
  write.csv(units, 'BODCdata/units.csv', row.names = FALSE, na = "" )
  write.csv(values, 'BODCdata/values.csv', row.names = FALSE, na = "" )
  
  
  BODC$parameters <- parameters
  BODC$units <- units
  BODC$values <- values
